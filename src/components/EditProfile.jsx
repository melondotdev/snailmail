...
const EditProfile = ({ userData, setIsPopupOpen, isPopupOpen, saveData }) => {
      ...
      <div className="popup-container absolute w-4/6 h-4/6 bg-gray-800 rounded-3xl flex z-40" style={{top: "10%", left: "15%"}}>
        <div className="profile-container flex flex-col items-center justify-between my-4 mx-4 w-2/5 font-inter rounded-3xl bg-cover bg-center z-50" style={{backgroundImage: `url(${BlurBg})`}}>
          <div className="pfp m-6 w-full flex flex-col items-center">
            <img src={DefaultPfp} alt='default' className="mb-4 w-1/2"></img>
            <h1 className="font-bold text-lg mb-1">{userData.docs[0].data().name}</h1>
            <h1 className="text-base opacity-80 w-full break-words px-4 text-sm overflow-y-auto">{userData.docs[0].data().about}</h1>
          </div>
